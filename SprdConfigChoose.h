...
#include <utils/Log.h>
#include "v1.x/SprdDisplayDevice.h"

#define HAL_PIXEL_FORMAT_YCbCr_420_SP 0x15
...
static inline bool getSPRDWindowSurfaceConfig(EGLDisplay display, android::DisplayDevice::DisplayType type,
                     EGLConfig* config, ANativeWindow* window)
{
    UNUSED(display);
    UNUSED(config);
    if (type >= HWC_DISPLAY_VIRTUAL)
    {
        int wformat = 0;
        window->query(window, NATIVE_WINDOW_FORMAT, &wformat);
        if((wformat != HAL_PIXEL_FORMAT_YCbCr_420_888) &&
           (wformat != HAL_PIXEL_FORMAT_YV12) &&
           (wformat != HAL_PIXEL_FORMAT_YCbCr_420_SP) &&
           (wformat != HAL_PIXEL_FORMAT_YCrCb_420_SP)){
            return true;
        }
        else
        {
