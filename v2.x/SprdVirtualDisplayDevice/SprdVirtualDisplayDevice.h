...
class SprdVirtualDisplayDevice {
 public:
  ...
  int32_t /*hwc2_error_t*/ CREATE_VIRTUAL_DISPLAY(
   ...
   int32_t /*hwc2_error_t*/ DESTROY_VIRTUAL_DISPLAY(SprdDisplayClient *Client);

  uint32_t GET_MAX_VIRTUAL_DISPLAY_COUNT(void);

  int32_t /*hwc2_error_t*/ ACCEPT_DISPLAY_CHANGES(SprdDisplayClient *Client);

  int32_t /*hwc2_error_t*/ CREATE_LAYER(SprdDisplayClient *Client, hwc2_layer_t* outLayer);
  ...
  int32_t SET_OUTPUT_BUFFER(SprdDisplayClient *Client, buffer_handle_t buffer,
   ...
   * */
  int buildSyncData(SprdDisplayClient *Client, 
          DisplayTrack *tracker, int32_t* outRetireFence);
 ...
 private:
  SprdDisplayClient *mClient[MAX_VDISPLAY_CLIENT];
  ...
  inline SprdVDLayerList *getHWLayerObj(SprdDisplayClient *client)
  {
    return static_cast<SprdVDLayerList *>(client->getUserData());
  }
};
