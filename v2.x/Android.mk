...
LOCAL_SHARED_LIBRARIES += vendor.sprd.hardware.enhance@1.0\
			  libbinder \
			  libhidlbase
endif

LOCAL_SRC_FILES := SprdHWComposer2.cpp \
		   SprdDisplayCore.cpp \
		   AndroidFence.cpp \
		   SprdDisplayPlane.cpp \
		   SprdHWLayer.cpp \
		   SprdDisplayDevice.cpp \
		   SprdHandleLayer.cpp \
...
LOCAL_C_INCLUDES := \
	...
	$(TOP)/vendor/sprd/modules/hwcomposer \
...
ifeq ($(strip $(POWER_HINT_VIDEO_LOWPOWER_DISPLAY)) , true)
    LOCAL_CFLAGS += -DUPDATE_SYSTEM_FPS_FOR_POWER_SAVE
endif

ifeq ($(strip $(ENABLE_PENDING_RELEASE_FENCE_FEATURE)) , true)
    LOCAL_CFLAGS += -DENABLE_PENDING_RELEASE_FENCE_FEATURE
endif
...
LOCAL_CFLAGS += -DMALI_DDK
else ifeq ($(strip $(TARGET_GPU_PLATFORM)),midgard)
LOCAL_CFLAGS += -DGRALLOC_MIDGARD
LOCAL_CFLAGS += -DMALI_DDK
else ifeq ($(strip $(TARGET_GPU_PLATFORM)),rogue)
LOCAL_CFLAGS += -DGRALLOC_ROGUE
LOCAL_CFLAGS += -DROGUE_DDK
...
ifeq ($(strip $(TARGET_SUPPORT_ADF_DISPLAY)),true)
    ...
    SPRD_DISPLAY_USE_DRM := true
ifeq ($(strip $(SPRD_DISPLAY_USE_DRM)),true)
    LOCAL_SHARED_LIBRARIES += libdrm

    LOCAL_SRC_FILES += \
    drm/SprdDrm.cpp \
    drm/drmresources.cpp \
    drm/drmcrtc.cpp \
    drm/drmencoder.cpp \
    drm/drmplane.cpp \
...
ifeq ($(strip $(TARGET_GPU_PLATFORM)),gondul)
    ...
    # FORCE_OVC force overlay composer process video composition.
    # LOCAL_CFLAGS += -DFORCE_OVC
...
ifeq ($(strip $(TARGET_REMOVE_OVERLAY_BUFFER)), true)
    LOCAL_CFLAGS += -DDUAL_PLANE_BUFFER
endif

# allocate(GSP) memory from system heap
# the change already in gralloc
#ifeq ($(strip $(MALI_PLATFORM_NAME)),sharkl3)
#LOCAL_CFLAGS += -DREMOVE_OVERLAY_BUFFER
#endif
