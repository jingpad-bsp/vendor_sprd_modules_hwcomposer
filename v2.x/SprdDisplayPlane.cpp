...
native_handle_t* SprdDisplayPlane:: createPlaneBuffer(int index)
{
    ...
    // only input buffer enable FBC/DC for FBC/DC temporary
    mPlaneUsage |= GRALLOC_USAGE_HW_RENDER;
    GraphicBufferAllocator::get().allocate(mWidth, mHeight, mFormat, 1,
                             mPlaneUsage, (buffer_handle_t*)&BufHandle,
                             &stride, 0x689423, reqname);
#ifdef SPRD_SR
    ALOGI("SPRD_SR SprdDisplayPlane::createPlaneBuffer allocated handle:%p", (void *)BufHandle);
#endif
    if (BufHandle == NULL)
    {
...
#ifdef SPRD_SR
    if(mSlots[index].mBuffer != NULL)
    {
#ifdef SPRD_SR_DEFER_FREE_OVB
        mDeferSlots.push_back(mSlots[index]);
        mDeferCount = 0;
#else
        if(mSlots[index].fenceFd > -1)
        {
            String8 name;
            name.append("RecreateOVBuffer");
            FenceWaitForever(name, mSlots[index].fenceFd);
            closeFence(&(mSlots[index].fenceFd));
        }
        ALOGI("SPRD_SR SprdDisplayPlane::createPlaneBuffer free mSlots[%d].mBuffer:%p", index, (void *)mSlots[index].mBuffer);
        GraphicBufferAllocator::get().free(static_cast<buffer_handle_t>(mSlots[index].mBuffer));
        mSlots[index].mBuffer = NULL;
#endif
    }
#endif
    mSlots[index].mBuffer = static_cast<native_handle_t* >(BufHandle);
...
native_handle_t* SprdDisplayPlane::dequeueBuffer(int *fenceFd)
{
        ...
        for(int i = 0; i < mBufferCount; i++)
        {
            ...
            switch (state)
            {
                ...
                case BufferSlot::FREE:
                {
                    ...
                    {
#ifdef SPRD_SR
                        if (mDisplayBufferIndex < 0 || i != mDisplayBufferIndex)
#endif
                            found = i;
                    }
                    break;
    ...
    native_handle_t* buffer = mSlots[found].mBuffer;
    ...
    if(mDeferCount>20 && mDeferSlots.size()>0)
    {
        for(auto slt : mDeferSlots)
        {
            if(slt.fenceFd > -1)
            {
                String8 name;
                name.append("DeferFreeOVBuffer");
                FenceWaitForever(name, slt.fenceFd);
                closeFence(&(slt.fenceFd));
            }
            ALOGI("SPRD_SR SprdDisplayPlane::dequeueBuffer defer free slt.mBuffer:%p", (void *)slt.mBuffer);
            GraphicBufferAllocator::get().free(static_cast<buffer_handle_t>(slt.mBuffer));
            slt.mBuffer = NULL;
        }
        mDeferSlots.clear();
        mDeferCount = -1;
    ...
    if((buffer == NULL && mWidth > 1 && mHeight > 1)
        ||(static_cast<unsigned int>(ADP_WIDTH(buffer)*ADP_HEIGHT(buffer)) != mWidth*mHeight))
    {
...
native_handle_t* SprdDisplayPlane::flush(int *fenceFd)
{
    ...
    int index(*front);
    native_handle_t* flushingBuffer = mSlots[index].mBuffer;
     ...
     * */
    mLastFlushedIndex = mFlushingBufferIndex;
    mFlushingBufferIndex = index;
    ALOGI_IF(mDebugFlag, "mFlushingBufferIndex:%d, mDisplayBufferIndex:%d, mLastFlushedIndex:%d",
               mFlushingBufferIndex, mDisplayBufferIndex, mLastFlushedIndex);
...
int SprdDisplayPlane::addFlushReleaseFence(int fenceFd)
{
  ...
  if (fenceFd < 0) {
    ALOGI_IF(mDebugFlag,
             "SprdDisplayPlane:: addReleaseFence cannot add invalid fence");
    return 0;
  }

  if (mSlots[mFlushingBufferIndex].mBufferState != BufferSlot::FLUSHED) {
    ALOGI("SprdDisplayPlane:: addReleaseFence BufferState wrong, cannot add "
          "fence, mFlushingBufferIndex: %d, BufferState: %d",
  ...
  if (mLastFlushedIndex >= 0) {
    int *acq_fd = &(mSlots[mLastFlushedIndex].fenceFd);
    closeFence(acq_fd);
    new_fd = dup(fenceFd);
    mSlots[mLastFlushedIndex].fenceFd = new_fd;

    ALOGI_IF(mDebugFlag, "SprdDisplayPlane:: addReleaseFence "
                         "mFlushingBufferIndex: %d, BufferState: %d, fenceFd: "
                         "%d, acuireFd: %d, newFd:%d",
             mLastFlushedIndex, (mSlots[mLastFlushedIndex].mBufferState),
             fenceFd, *acq_fd, new_fd);
  }
...
bool SprdDisplayPlane::close()
{
    ...
    for (int i = 0; i < mBufferCount; i++)
    {
        native_handle_t* bufferHandle = mSlots[i].mBuffer;
