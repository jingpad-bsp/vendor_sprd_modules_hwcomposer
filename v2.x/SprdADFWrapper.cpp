...
inline void implementBufferFormatConfig(int androidFormat,
                                        struct adf_buffer_config *bufferConfig,
                                        native_handle_t *privateH) {
  int Bpf = ADP_STRIDE(privateH)*ADP_HEIGHT(privateH);
  bufferConfig->fd[0] = ADP_BUFFD(privateH);
  bufferConfig->offset[0] = 0;
  bufferConfig->n_planes = 1;
  bufferConfig->pitch[0] = ADP_STRIDE(privateH)*4;
  switch (androidFormat) {
    ...
    case HAL_PIXEL_FORMAT_RGB_888:
      bufferConfig->pitch[0] = ADP_STRIDE(privateH)*3;
      bufferConfig->format = DRM_FORMAT_RGB888;
      break;
    case HAL_PIXEL_FORMAT_RGB_565:
      bufferConfig->pitch[0] = ADP_STRIDE(privateH)*2;
      bufferConfig->format = DRM_FORMAT_RGB565;
      break;
    case HAL_PIXEL_FORMAT_YCbCr_420_SP:
      bufferConfig->n_planes = 2;
      bufferConfig->fd[1] = ADP_BUFFD(privateH);
      bufferConfig->offset[1] = Bpf;
      bufferConfig->pitch[0] = ADP_STRIDE(privateH) * 1;
    ...
    case HAL_PIXEL_FORMAT_YV12:
      ...
      bufferConfig->fd[1]          = ADP_BUFFD(privateH);
      bufferConfig->fd[2]          = ADP_BUFFD(privateH);
      bufferConfig->offset[1]      = Bpf;
      bufferConfig->offset[2]      = Bpf+ALIGN(ADP_STRIDE(privateH)/2, 16)*ADP_HEIGHT(privateH)/2;
      bufferConfig->pitch[0]       = ADP_STRIDE(privateH)*1;
      bufferConfig->pitch[1]       = ALIGN(ADP_STRIDE(privateH)/2, 16);
    ...
    default:
      ALOGW(
          "SprdADFWrapper::implementBufferFormatConfig() don't supported "
          "format:%x",
          androidFormat);
...
bool SprdADFWrapper::Init() {
  ...
  static const struct adf_hwc_event_callbacks Callbacks = {
      .vsync = SprdEventHandle::SprdHandleVsyncReport,
      .hotplug = NULL/*SprdEventHandle::SprdHandleHotPlugReport*/,
      .custom_event = SprdEventHandle::SprdHandleCustomReport,
  };

  if (SprdDisplayCore::Init() == false) {
    ALOGE("SprdADFWrapper:: Init SprdDisplayCore::Init failed");
    goto ERR0;
  }
  ...
  ALOGI_IF(mDebugFlag, "SprdADFWrapper:: Init success find interface num: %d",
           mNumInterfaces);
...
int SprdADFWrapper::GetConfigs(int DisplayType, uint32_t *Configs,
                               size_t *NumConfigs) {
  int ret = -1;
  ...
  ALOGD("SPRD_SR SprdADFWrapper:: GetConfigs , [%d,%d,%d]", Configs[0],Configs[1],Configs[2]);
  return 0;
}

int SprdADFWrapper::setActiveConfig(int DisplayType, uint32_t Config) {
  int ret = -1;
  ...
  ALOGI("SPRD_SR SprdADFWrapper:: setActiveConfig: %d", Config);
  ret = adf_set_active_config_hwc2(mHelper, DisplayType, Config);
  if (ret) {
    ALOGE("SPRD_SR SprdADFWrapper:: setActiveConfig error, ret: %d", ret);
    return -1;
  }
  return 0;
}

int SprdADFWrapper::getActiveConfig(int DisplayType, uint32_t *pConfig) {
  int ret = -1;
  ...
  ret = adf_get_active_config_hwc2(mHelper, DisplayType, pConfig);
  if (ret) {
    ALOGE("SPRD_SR SprdADFWrapper:: getActiveConfig error, ret: %d", ret);
    return -1;
  }
  ALOGD("SPRD_SR SprdADFWrapper:: getActiveConfig: %d", *pConfig);
  return 0;
}

int SprdADFWrapper::GetConfigAttributes(int DisplayType, uint32_t Config,
  ...
  ret = adf_getDisplayAttributes(mHelper, DisplayType, Config, attributes,
                                 values);
...
int SprdADFWrapper::Blank(int DisplayType, bool enabled) {
  ...
  int durationMillis = (int)nanoseconds_to_milliseconds(systemTime(SYSTEM_TIME_BOOTTIME) - start);
  if (durationMillis > 100) {
      ALOGD("Excessive delay in adf_blank(): %dms", durationMillis);
  }
...
int SprdADFWrapper::Dump(char *buffer) {
  if (mInitFlag == false) {
    ALOGE("func: %s line: %d SprdADFWrapper Need Init first,buffer:%p",
...
int SprdADFWrapper::createADFInterface() {
  ...
  if (mNumInterfaces <= 0) {
    ALOGE(
        "SprdADFWrapper:: createADFInterface adf_interface failed "
        "mNumInterfaces: %d",
        mNumInterfaces);
    goto ERR0;
  }
  ...
  for (i = 0; i < mNumInterfaces; i++) {
    ...
    if (mInterfaces[i].Fd < 0) {
      ALOGE("SprdADFWrapper:: createADFInterface adf_interface_open: %d failed",
            i);
      goto ERR2;
    }

    if (adf_get_interface_data(mInterfaces[i].Fd, &(mInterfaces[i].Data))) {
      ALOGE("SprdADFWrapper:: createADFInterface adf_get_interface_data i: %d",
            i);
      goto ERR2;
    }
  ...
  for (i = 0; i < mNumInterfaces; i++) {
    ...
    pInter->numEngines = adf_overlay_engines_for_interface(
        mDevice, mInterfaceIDs[i], &pInter->EngineIDs);
    if (pInter->numEngines <= 0) {
      ALOGE(
          "SprdADFWrapper:: createADFInterface interface %d has no overlay "
          "engine attached",
          mInterfaceIDs[i]);
      goto ERR2;
    }

    pInter->Engines = (ADF_overlay_engine *)calloc(pInter->numEngines,
                                                   sizeof(ADF_overlay_engine));
    if (pInter->Engines == NULL) {
      ALOGE("SprdADFWrapper:: createADFInterface calloc Engines i: %d failed",
            i);
      goto ERR2;
    }
    ...
    for (j = 0; j < pInter->numEngines; j++) {
      ...
      pEngine->Fd =
          adf_overlay_engine_open(mDevice, pInter->EngineIDs[j], O_RDWR);
      if (pEngine->Fd < 0) {
        ALOGE(
            "SprdADFWrapper:: createADFInterface adf_overlay_engine_open "
            "inter: %d, engine: %d",
            i, j);
        goto ERR2;
      }
      ...
      if (adf_get_overlay_engine_data(pEngine->Fd, &pEngine->Data)) {
        ALOGE(
            "SprdADFWrapper:: createADFInterface adf_get_overlay_engine_data "
            "inter: %d, engine: %d",
            i, j);
        goto ERR2;
      }
...
void SprdADFWrapper::DestroyADFSignalHandler(int Signal, siginfo_t *SigInfo,
                                             void *user) {
  ALOGI_IF(g_debugFlag, "Signal:%d,SigInfo:%p,user:%p", Signal, SigInfo, user);
}

int SprdADFWrapper::AddFlushData(int DisplayType, SprdHWLayer **list,
                                 int LayerCount) {
  int i = 0;
  ...
  ALOGI_IF(mDebugFlag, "SprdADFWrapper:: AddFlushData disp: %d, LayerCount: %d",
...
int SprdADFWrapper::PostDisplay(DisplayTrack *tracker) {
  ...
  if (mLayerCount <= 0) {
    ALOGI_IF(mDebugFlag, "SprdADFWrapper:: PostDisplay No Layer should be displayed");
    ret = -1;
    goto EXT0;
  }

  BufferConfig = (struct adf_buffer_config *)calloc(
      mLayerCount, sizeof(struct adf_buffer_config));
  ...
  iCustomDataSize = sizeof(struct sprd_adf_post_custom_data) +
                    sizeof(struct sprd_adf_hwlayer_custom_data) * mLayerCount;
  ...
  for (i = 0; i < mActiveContextCount; i++) {
    ...
    if (attachOverlayEngineToInterface(ctx, &overlayEngineId)) {
      ALOGE(
          "SprdADFWrapper:: PostDisplay attachOverlayEngineToInterface failed");
      ret = -1;
      goto EXT2;
    }
    ...
    for (j = 0; j < ctx->LayerCount; j++) {
      ...
      if (implementBufferConfig(l, overlayEngineId, &(BufferConfig[index]))) {
      ...
      ALOGI_IF(mDebugFlag, "SprdADFWrapper:: PostDisplay config %dth layer",
               index);
    }
  ...
  static int f_c = 0;
  ALOGI_IF(mDebugFlag,
           "adf_device_post() %d times, interfaceNum:%d, Layer count: %d",
           f_c++, interfaceNum, mLayerCount);

#ifdef SPRD_SR
  static uint32_t boot_finished = 0;
  static int32_t lcd_size[2] = {0, 0};

  if (!boot_finished) {
    if (lcd_size[0] == 0) {
      uint32_t attributes[3];
      //panel size is stored in display_configs[0]
      attributes[0] = HWC_DISPLAY_WIDTH;
      attributes[1] = HWC_DISPLAY_HEIGHT;
      attributes[2] = HWC_DISPLAY_NO_ATTRIBUTE;
      adf_getDisplayAttributes(mHelper, DISPLAY_PRIMARY, 0, attributes, lcd_size);
      ALOGI_IF(mDebugFlag,
              "SprdADFWrapper:: PostDisplay lcd width:%d height:%d", lcd_size[0], lcd_size[1]);
    }

    //there is single layer in bootanimation
    if (mLayerCount == 1) {
      if ((BufferConfig[0].w == lcd_size[0])
      && (BufferConfig[0].h == lcd_size[1])
      && (adfLayers[0].dst_w < BufferConfig[0].w)
      && (adfLayers[0].dst_h < BufferConfig[0].h)) {
          adfLayers[0].start_w = BufferConfig[0].w;
          adfLayers[0].start_h = BufferConfig[0].h;
          adfLayers[0].dst_w = BufferConfig[0].w;
          adfLayers[0].dst_h = BufferConfig[0].h;
          ALOGI_IF(mDebugFlag,
                  "SprdADFWrapper:: PostDisplay start(%d %d) dst(%d %d) bufferConfig(%d %d)",
                  adfLayers[0].start_w, adfLayers[0].start_h,
                  adfLayers[0].dst_w, adfLayers[0].dst_h,
                  BufferConfig[0].w, BufferConfig[0].h);
        }
      } else {
        boot_finished = 1;
        ALOGI_IF(mDebugFlag,
              "SprdADFWrapper:: PostDisplay boot_finished");
      }
  }
#endif
  //ret = adf_device_post(mDevice, mInterfaceIDs, interfaceNum, BufferConfig,
  //                      mLayerCount, custom, iCustomDataSize);

  ret = adf_device_post_v2(mDevice, mInterfaceIDs, interfaceNum, BufferConfig, (uint32_t)mLayerCount, custom, iCustomDataSize,
                                                  ADF_COMPLETE_FENCE_PRESENT, &(tracker->releaseFenceFd));

  if (ret < 0) {
    ALOGE("SprdADFWrapper:: PostDisplay adf_device_post error: %d,%d", ret,
          NUM_FB_BUFFERS);
    goto EXT3;
  }
  ...
  if (fllow_control_rel_fds.size() > (NUM_FB_BUFFERS + 1)) {
    ...
    String8 name("SprdADFWrapper:fllow_control");
  ...
  ALOGI_IF(mDebugFlag,
           "<05> af adf_device_post() return rel_fd:%d,retire_fd:%d,",
...
int SprdADFWrapper::attachOverlayEngineToInterface(FlushContext *ctx,
                                                   adf_id_t *overlayEngineId) {
  int i = 0;
  int err = 0;
...
void SprdADFWrapper::detachOverlayEngineFromInterfaces(
    struct adf_buffer_config **bufferConfig) {
  int i = 0;
...
int SprdADFWrapper::implementBufferConfig(
    SprdHWLayer *l, adf_id_t overlayEngineId,
    struct adf_buffer_config *bufferConfig) {
  int format = -1;
  ...
  bufferConfig->w = ADP_WIDTH(privateH);
  bufferConfig->h = ADP_HEIGHT(privateH);
  ...
  ALOGI_IF(mDebugFlag,
           "SprdADFWrapper:: implementBufferConfig w:%d, h:%d,stride: %d, "
           "format: %d, bufFd: %d, acfence:%d, OVEnId: %d",
           bufferConfig->w, bufferConfig->h, ADP_STRIDE(privateH),
           format, ADP_BUFFD(privateH),
           bufferConfig->acquire_fence, bufferConfig->overlay_engine);

  return 0;
}

void SprdADFWrapper::implementCustomConfig(
    FlushContext *ctx, SprdHWLayer *l,
    struct sprd_adf_hwlayer_custom_data *adfLayer, int32_t index) {
  native_handle_t *privateH = NULL;
  ...
  struct sprdRectF *src = l->getSprdSRCRectF();
  ...
  adfLayer->dst_w = fb->w;
  adfLayer->dst_h = fb->h;
  adfLayer->start_x = src->x;
  adfLayer->start_y = src->y;
  adfLayer->start_w = src->w;
  adfLayer->start_h = src->h;
  adfLayer->blending = l->getBlendMode();
  adfLayer->rotation = convertToADFTransform(l->getTransform());
  adfLayer->scale = 0;        // ?
  adfLayer->compression = ADP_COMPRESSED(privateH) ? ADF_COMPRESSED : ADF_UNCOMPRESSED;
  adfLayer->height = ADP_COMPRESSED(privateH) ? ADP_VSTRIDE(privateH) :
    ADP_HEIGHT(privateH);
  adfLayer->header_size_r = ADP_HEADERSIZER(privateH);
  adfLayer->header_size_y = ADP_HEADERSIZEY(privateH);
  adfLayer->header_size_uv = ADP_HEADERSIZEUV(privateH);

  ALOGI_IF(mDebugFlag,
           "SprdADFWrapper:: implementCustomConfig interfaceId:%d,hwlayerid: "
           "%d, bufFd: %d, planeAlpha:%d, dst(x:%d,y:%d,w:%d,h:%d) blending: "
           "%x, transform: %d, scale: %d, compression: %d headerSizeR: 0x%x",
           ...
           adfLayer->scale, adfLayer->compression, ADP_HEADERSIZER(privateH));
}
