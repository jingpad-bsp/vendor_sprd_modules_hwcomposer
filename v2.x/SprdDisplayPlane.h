...
#ifdef SPRD_SR
/*DEFER_FREE_OVB fix screen stuck problem, when switch resolution frequently.
that is caused by fence-wait when RecreateOVBuffer.
*/
#define SPRD_SR_DEFER_FREE_OVB
#endif
/*
 ...
 * */
struct BufferSlot {
  BufferSlot() : mBufferState(BufferSlot::FREE), mTransform(0), mBuffer(NULL), fenceFd(-1) {}
...
class SprdDisplayPlane {
 ...
 private:
  ...
  //BufferSlot mDeferSlots[PLANE_BUFFER_NUMBER];//for defer free
  Vector<BufferSlot> mDeferSlots;
  int mDeferCount;
#endif
  int mDisplayBufferIndex;
  int mFlushingBufferIndex;
  int mLastFlushedIndex;
