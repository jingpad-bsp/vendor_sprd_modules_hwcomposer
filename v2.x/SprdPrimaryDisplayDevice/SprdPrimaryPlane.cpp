...
void SprdPrimaryPlane::AttachPrimaryLayer(SprdHWLayer *l,
  ...
  if (DirectDisplayFlag) {
    SetDisplayParameters(mHWLayer);
  }
}

void SprdPrimaryPlane::AttachFramebufferTargetLayer(
    SprdHWLayer *FBTargetLayer) {
  native_handle_t *privateH = (FBTargetLayer == NULL) ?
                                       NULL : FBTargetLayer->getBufferHandle();
...
bool SprdPrimaryPlane::SetDisplayParameters(SprdHWLayer *Layer) {
  if (Layer == NULL) {
  ...
  native_handle_t *privateH = Layer->getBufferHandle();
  ...
  if ((ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCbCr_420_SP) ||
      ...
      (ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCbCr_420_888) ||
      (ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YV12) ||
      (ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED)) {
    ALOGI(
