...
#ifdef SPRD_SR
int SprdFrameBufferDevice::setActiveConfig(int DisplayType, uint32_t Config) {
  return 0;
}
#endif
...
int SprdFrameBufferDevice::Dump(char *buffer) {
  HWC_IGNORE(buffer);
  return 0;
}
...
int SprdFrameBufferDevice::configPrimaryPlane(FlushContext_t *ctx,
  ...
  MemIon::Get_phy_addr_from_ion(privateH->share_fd, &(privateH->phyaddr),
                                       &size);
...
int SprdFrameBufferDevice::frameBufferDevicePost() {
  ...
  if (mFBTDisplay) {
    ...
    if (mFBTLayer->getAcquireFence() >= 0) {
      String8 name("HWCFBT::Post");

      FenceWaitForever(name, mFBTLayer->getAcquireFence());
      closeFence(mFBTLayer->getAcquireFencePointer());
    }
