...
int SprdHWLayerList:: updateGeometry(hwc_display_contents_1_t *list, int accelerator)
{
   ...
   mSprdLayerList = new SprdHWLayer*[mLayerCount];
   if (mSprdLayerList == NULL)
    {
      ALOGE("Cannot create mSprdLayerList");
      return -1;
    }
    ...
    mDispLayerList = new SprdHWLayer*[mLayerCount];
    if (mDispLayerList == NULL)
    {
        ALOGE("Cannot create DispLayerList");
        return -1;
    }
    ...
    for (unsigned int i = 0; i < mLayerCount; i++)
    {
        ...
        mSprdLayerList[mSprdLayerCount++] = &(mLayerList[i]);
    ...
    {
        if (((mLayerCount > 1) && mAccerlator)&& (mSkipLayerFlag == false))
        {
            ret = mAccerlator->Prepare(mSprdLayerList, mSprdLayerCount -1, mGXPSupport);
            ...
            for (unsigned int i = 0; i < mLayerCount; i++)
            {
              if ((mLayerList[i].getAccelerator() == ACCELERATOR_GSP) && mGXPSupport)
              {
                unsigned int count   = mGXPLayerCount;
                mGXPLayerList[count] = &(mLayerList[i]);
                mGXPLayerCount++;
              ...
              else if (mLayerList[i].getAccelerator() == ACCELERATOR_DISPC)
              {
                unsigned int count    = mDispCLayerCount;
                mDispLayerList[count] = &(mLayerList[i]);
                mDispCLayerCount++;
