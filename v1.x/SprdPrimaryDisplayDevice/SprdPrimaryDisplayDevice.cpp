...
#ifdef ACC_BY_DISPC
  if (DispCLayerCount > 0 &&
      (DispCLayerCount == mLayerList->getSprdLayerCount() - 1)) {
...
int SprdPrimaryDisplayDevice::WrapOverlayLayer(native_handle_t *buf, int format,
                                               int fenceFd,
                                               hwc_display_contents_1_t *list) {
  struct sprdRect *src = NULL;
  struct sprdRect *fb = NULL;
  ...
  mComposedLayer = new SprdHWLayer(buf, format, 255, OVTargetLayer->blending,
                                   0x00, fenceFd);  // HWC_BLENDING_NONE
  src = mComposedLayer->getSprdSRCRect();
  fb = mComposedLayer->getSprdFBRect();
...
int SprdPrimaryDisplayDevice::buildSyncData(hwc_display_contents_1_t *list,
  ...
  if (tracker->releaseFenceFd == -1 || tracker->retiredFenceFd == -1) {
    ALOGI_IF(mDebugFlag,
             "SprdPrimaryDisplayDevice:: buildSyncData input fencefd illegal");
    return -1;
  }
...
FBTPath:
  ...
  if (BufferSyncBuild2(mLayerList->getSprdLayerList(),
                       mLayerList->getSprdLayerCount(), HWCReleaseFenceFd,
                       DisplayReleaseFenceFd, tracker->retiredFenceFd,
                       &list->retireFenceFd)) {
